...
class UserProfileSignalTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = self.User.objects.create_user(username='testuser', email='testemail@mail.com', password='testpassword')
        self.admin = self.User.objects.create_superuser(username='testadmin', email='admin@mail.com', password='adminpass')
    ...
    def test_user_must_have_role(self):
        ...
        with self.assertRaises(Exception):
            self.user = self.User.create_user(username='norole', email='norole@mail.com', password='norolepass', role=None)
    ...
    def test_cannot_create_user_without_username(self):
        with self.assertRaises(ValueError):
            self.User.objects.create_user(username='', email='testuser@gmail.com', password='testpass')
    
    def test_cannot_create_user_with_invalid_email(self):
        with self.assertRaises(ValueError):
            self.User.objects.create_user(username='invalidemail', email='not-email', password='passtest')

    def test_cannot_create_user_with_duplicate_email(self):
        # Tests to check no new user is created with existing email address
        with self.assertRaises(Exception):
            self.user = self.User.objects.create(username='newuser', email=self.user.email, password='newpass')
    
    def test_no_profile_for_superusers(self):
        #Test for no profile should be created for superusers
        self.admin_user = self.User.objects.create_superuser(username='admin2', email='admin2@mail.com', password='admin1testpass2')
